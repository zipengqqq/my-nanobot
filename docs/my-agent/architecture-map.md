...
这份文档不是复述整个 `nanobot` 仓库，而是抽取出“`my_agent` 要发展成完整 agent 框架时，必须长期保留的核心架构”。

第一阶段里，我们是为了做出最小 CLI 同构版而阅读它；现在第一阶段已经完成，后续所有学习和开发仍然以这份文档为准，但用途已经变成：

- 识别哪些层是 `my_agent` 长期要保留的内核
- 判断新能力属于核心层、运行时层，还是扩展生态层
- 防止第二阶段产品化扩展时破坏已经验证完成的主链路
...
也就是说，我们先跳过了“多渠道适配”，只保留“统一消息对象”这个思想。

进入第二阶段后，这一层的演进重点会变成：

- 抽出统一入口模型
- 让 CLI / API / WebUI / 多渠道入口共享同一条主链路
- 尽量不要把渠道差异泄漏进 `AgentLoop`
...
- 先恢复并扩展常用开发工具
- 再补 Web / exec session / patching 等高频能力
- 最后再决定是否引入 MCP、cron、spawn 等产品级工具生态
...
`my_agent` 当前不应再被定义为“最小 CLI agent 学习项目”，而应被定义为：

> 一个参考 `nanobot` 架构实现、以完整 agent 框架为目标、当前已完成核心最小闭环验证的独立项目。

这意味着：

- 第一阶段已经完成
- 当前内核已经可以作为长期基座继续扩展
- 后续工作重点从“证明主链路成立”切换为“控制复杂度地补齐产品能力”
...
它们没有进入第一阶段，不是因为不重要，而是因为它们属于“产品规模扩展”。
...
我们现在不会再把 `my_agent` 当成“最小练手项目”重复构建，而是：

- 以 `nanobot` 为参考架构
- 保留第一阶段已经验证完成的核心分层
- 在此基础上逐步扩展成完整 agent 框架
- 每一轮都按这份架构地图校正方向

这能最大限度降低第二阶段产品化扩展带来的架构漂移。
